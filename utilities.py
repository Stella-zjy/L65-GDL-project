...
def prepare_syn_data(G, labels, train_split, if_adj=False):
    ...
    return {"x": features, "y": labels, "edges": edges, "edge_list": edge_list, "train_mask": train_mask,
            "test_mask": test_mask}


# change the batch size for small loader 0215
def prepare_real_data(graphs, train_split, batch_size, dataset_str, small_loader_batch_size):
    ...
    if dataset_str == "Mutagenicity":
        ...
        small_loader = DataLoader(test_set, batch_size=small_loader_batch_size)
...
def get_top_subgraphs_k(top_indices, y, edges, num_expansions, graph_data=None, graph_name=None):
    # num_expansions 4
    graphs = []
    color_maps = []
    labels = []
...
def get_node_distances(clustering_model, data):
    # return sorted distances of data points to cluster centerssorted distances of data points to cluster centers
...
def plot_samples(clustering_model, data, y, layer_num, k, clustering_type, reduction_type, num_nodes_view, edges,
    ...
    # rows (k) and columns (col) of the subplot grid
    ...
    for i, ax_list in zip(l, axes):
        ...
        for view in num_nodes_view:
            ...
            # top_indices shape (num_nodes_view, )

            # get_top_subgraphs get the top 5 subgraphhs of 4-hop in one cluster
            tg, cm, labels, node_labels = get_top_subgraphs_k(top_indices, y, edges, num_expansions, graph_data,
                                                              graph_name)
    ...
    if graph_data is not None:
        if isinstance(clustering_model, AgglomerativeClustering):
            ...
            fig2.savefig(
                os.path.join(path, f"{layer_num}layer_{clustering_type}_{reduction_type}_{views}view_by_node.png"))
...
def plot_dendrogram(data, reduction_type, layer_num, path):
    ...
    dendrogram = hierarchy.dendrogram(hierarchy.linkage(data, method='average'), truncate_mode="lastp", ax=ax,
                                      leaf_rotation=90.0, leaf_font_size=14)
    ax.set_xlabel("Nodes")
...
def plot_completeness_table(model_type, calc_type, data, path):
    ...
    ax.axis('off')
    ax.table(cellText=data, colLabels=headings, loc="center", rowLoc="center", cellLoc="center", colLoc="center",
             fontsize=18)
...
def prepare_output_paths(dataset_name, k):
    ...
    return {"base": path, "TSNE": path_tsne, "PCA": path_pca, "UMAP": path_umap, "KMeans": path_kmeans, "HC": path_hc,
            "Ward": path_ward, "DBSCAN": path_dbscan}


import numpy as np


def reshape_array_based_on_mask(input_array, mask):
    """
    Reshape an array based on a mask of 0s and 1s.

    Parameters:
    - input_array: A numpy array of shape (A, B, C).
    - mask: A numpy array of 0s and 1s of shape (A, B).

    Returns:
    - A numpy array of shape (N, C), where N is the number of 1s in the mask.
    """
    # Check if the mask is correctly shaped
    if input_array.shape[:2] != mask.shape:
        raise ValueError("Shape mismatch: input_array and mask must have compatible shapes.")

    # Flatten the mask to use it for filtering
    mask_flat = mask.flatten()

    # Flatten the first two dimensions of the input_array and apply the mask
    filtered_array = input_array.reshape(-1, input_array.shape[-1])[mask_flat == 1]

    return filtered_array


def replicate_elements(seq, times):
    """
    Replicate each element in a sequence a given number of times.

    Parameters:
    - seq: The original sequence of elements (list, tuple, etc.).
    - times: The number of times each element should be replicated.

    Returns:
    - A new list with each element of the original sequence replicated 'times' times.
    """
    replicated_seq = [element for element in seq for _ in range(times)]
